...
author:
  - "[http://rockefelleruniversity.github.io/Intro_To_R_1Day/](http://rockefelleruniversity.github.io/Intro_To_R_1Day/)"
...
- [Set up](#set-up)
- [Background of R](#background-of-r)
...
Once the zip file is unarchived, all presentations as HTML slides and pages, their R code and HTML practical sheets will be available in the directories underneath.

* **docs/presentations/slides/**
Presentations as an HTML slide show.
* **docs/presentations/singlepage/**
Presentations as an HTML single page.
* **docs/presentations/r_code/**
R code in presentations.
* **docs/exercises/exercises/**
Practicals as HTML pages.
* **docs/exercises/answers/**
Practicals with answers as HTML pages.
* **docs/data/**
Data files used in the presentations and practicals.

# Background of R {background-color="#23373B" .center .middle}
...
**R** provides access to 1000s of packages ([CRAN](https://cran.r-project.org/), [Bioconductor](https://bioconductor.org/), [R-universe](https://r-universe.dev/)) which extend the basic functionality of R while maintaining high quality documentation.

In particular, [Robert Gentleman](https://en.wikipedia.org/wiki/Robert_Gentleman_%28statistician%29) developed the **[Bioconductor](https://bioconductor.org/)** project where 100's of packages are directly related to computational biology and analysis of associated high-throughput experiments.
:::
...
![Bioconductor package downloads over time](../imgs/bioconductor_packages.png)
...
Freely available from the [R-project website](https://cran.r-project.org/).

RStudio provides an integrated development environment (IDE) which is freely available from [Posit](https://posit.co/download/rstudio-desktop/), the company that develops RStudio.
...
# Data Types in R {background-color="#23373B" .center .middle}
...
```{r simpleCalculations_introtoR,prompt=TRUE}
3+1
...
The complete list of functions that come with base R is in the [base package index](https://stat.ethz.ch/R-manual/R-devel/library/base/html/00Index.html).
...
```{r usingFunctionsSetNames_introtoR,eval=FALSE}
dir()
dir(full.names=T)
```
...
```{r usingFunctionsDefaultOrder_introtoR,eval=FALSE}
dir(full.names=T)
# Is equivalent to...
dir(".",NULL,FALSE,T)
```

# Variables {background-color="#23373B" .center .middle}
...
# Vectors {background-color="#23373B" .center .middle}
...
Vectors in R can be used in arithmetic operations as seen with [variables earlier](#using-variables). When a standard arithmetic operation is applied to vector, the operation is applied to each position in a vector.
...
In R we can also create character vectors [again using **c()** function](#vector-basics). These vectors can be indexed just the same.
...
**NA** values allow for R to handle missing data correctly as they require different handling than standard numeric or character values. We will illustrate an example handling **NA** values [later](#ordering-with-na-values).
...
Logical vectors are a class of vector made up of TRUE or FALSE boolean values. The single letters T/F also work, but TRUE/FALSE are safer: T and F are ordinary variables that can be overwritten, while TRUE and FALSE cannot.
...
Answers can be found [here](../../exercises/answers/Vectors_answers.html)

# Matrices {background-color="#23373B" .center .middle}

## Creating matrices

In programs such as Excel we are used to tables.

![A numeric table in Excel](../imgs/ExcelMat2.png){fig-align="center" height="300"}
...
## Joining incompatible vectors and matrices
...
[As we have seen with vectors](#character-vectors), matrices can be named. For matrices the naming is done by columns and rows using **colnames()** and **rownames()** functions.
...
Selecting and replacing portions of a matrix can be done by **indexing** using square brackets **[]** much [like for vectors](#vector-indexing).
...
```{r,matricesOneTypeOnlyNoError_introtoR,error=TRUE}
narrowMatrix[,2] *2
```
...
```{r matricesOneTypeOnlyWithError_introtoR,error=TRUE}
narrowMatrix[,2] *2
```
...
Answers can be found [here](../../exercises/answers/Matrices_answers.html)

# Factors {background-color="#23373B" .center .middle}
...
# Data Frames {background-color="#23373B" .center .middle}

## Creating data frames

[We saw that with matrices you can only have one type of data](#matrix-data-types). We tried to create a matrix with a character element and the entire matrix became a character.
...
![An Excel table mixing text, categories and numbers](../imgs/ExcelMat.jpg){fig-align="center" height="300"}
...
Data frames may be indexed just [as matrices](#matrix-indexing).
...
```{r dataframesIndexbyName_introtoR, eval=FALSE}
dfExample[,"Surv"]
```
...
When you create a data frame you can automatically convert all character columns to factors with the **stringsAsFactors** argument. Since R 4.0.0 the default is **FALSE**.
...
## Ordering with NA values {#ordering-with-na-values}
...
Answers can be found [here](../../exercises/answers/FactorsAndDataframes_answers.html)

# Lists {background-color="#23373B" .center .middle}
...
[Just as with vectors](#character-vectors), list elements can be assigned names.
...
List, as [with other data types in R](#vector-indexing) can be indexed. In contrast to other types, using **[]** on a list will subset the list to another list of selected indices. To retrieve an element from a list in R , two square brackets **[[]]** must be used.
...
Again, [similar to vectors](#vector-basics), lists can be joined together in R using the c() function
...
Exercise on lists can be found [here](../../exercises/exercises/Lists_exercise.html)

## Answers to exercise

Answers can be found [here](../../exercises/answers/Lists_answers.html)

# Coercing data classes {background-color="#23373B" .center .middle}
...
## Working with objects {.scrollable}
...
# Reading and Writing Data {background-color="#23373B" .center .middle}
...
![Knowing where you are is the first step to giving directions](../imgs/youarehere.png){height="420"}
...
![Relative (directions) versus absolute (an address) paths](../imgs/youarehere2.png){height="420"}
...
And the clipboard (Windows first, then macOS).
...
## Data from file columns {.scrollable}
...
## Reviewing your data {.scrollable}
...
We can make use of an R package (the rio package) which allows us to import and export data to multiple formats.
...
Exercise on reading and writing data can be found [here](../../exercises/exercises/DataInputOutput_exercise.html)
