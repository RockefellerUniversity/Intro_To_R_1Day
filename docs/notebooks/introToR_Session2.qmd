...
- [Libraries](#getting-additional-libraries)
- [Writing scripts](#scripts)

# Recap {background-color="#23373B" .center .middle}

## Session 1 recap

Session 1 covered introduction to R data types and import/export of data.

- [Background of R](../singlepage/introToR_Session1.html#background-of-r)
- [Data types in R](../singlepage/introToR_Session1.html#data-types-in-r)
- [Reading and writing data](../singlepage/introToR_Session1.html#reading-and-writing-data)
...
# Conditions and Loops {background-color="#23373B" .center .middle}
...
We may wish to keep track of the position in x we are evaluating to retrieve the same index in other variables. A common practice is to loop through all possible index positions of x using **seq_along(x)** (you will also often see **1:length(x)**, which does the same for non-empty vectors).
...
::: {.column width="50%"}
```{r,message=TRUE,eval=TRUE,echo=FALSE}
x <- 1:8
...
Answers can be found [here](../../exercises/answers/ConditionsAndLoops_answers.html)

# Custom functions {background-color="#23373B" .center .middle}
...
# Getting Additional Libraries {background-color="#23373B" .center .middle}
...
We can also install from other sources. For Bioinformatics we have **[Bioconductor](https://bioconductor.org/)**. There is a huge collection of packages that are interoperable and specific to our interest i.e.

Bioconductor packages undergo peer review, undergo rigorous testing and are required to have vignettes unlike CRAN packages so we typically hold these packages in a higher regard.
...
Again we must install a helper package from CRAN, here *remotes* (the lightweight package that *devtools* uses for this). We can then use the *install_github()* function along with the name of the GitHub repository to install the package.
...
# Scripts {background-color="#23373B" .center .middle}
...
The file **dayOfWeek.r** in the "data/scripts" directory contains a simple R script to tell you what day it is.
...
Answers can be found [here](../../exercises/answers/Functions_answers.html)
