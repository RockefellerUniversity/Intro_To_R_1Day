...
```
Warning in `[<-.factor`(`*tmp*`, 1, value = "huge") :
...
```{r}
format(myDate,format="%H O'Clock %p %A on %B %dth")
format(myDate,format="%H O'Clock %p %A on %B %dth",tz = "GMT")
```
...
By using these specialised objects we will be able to focus on understanding and analyzing the data contained within the object instead of how to format and interact with such complex data.
...
We can use the *saveRDS()* and *readRDS()* functions to save and read in our R object. They are save as a *.rds object. 
